...
var tags = [new Set(['cats', 'harnesses', 'featured']),
             new Set(['dogs', 'harnesses', 'featured']),
             new Set(['cats', 'dogs', 'accessories', 'featured']),
             new Set(['cats', 'dogs', 'accessories']),
             new Set(['cats', 'dogs', 'accessories']),
...
/**
  ...
  */
  getFilteredList = function(tag, items_dict) {
    var new_dict = [];
    for (var i=0; i<Object.keys(items_dict).length; i++) {
      var key = ids[i];
      var item = items_dict[key];
      if (item.hasTag(tag)) {
        new_dict[key] = item;
        console.log('Filtered by '+tag+': '+item.getName());
      }  
    }
    return new_dict;
  };
